...
func _ready():
	...
	if CameraServer.get_feed_count() > 0:
		...
		get_surface_override_material(0).set("shader_parameter/camera_y", cam_text_y)
		get_surface_override_material(0).set("shader_parameter/camera_CbCr", cam_text_CbCr)
	else:
